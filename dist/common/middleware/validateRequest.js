...
const logValidationDiagnostics = (req, error) => {
    if (process.env.LOG_VALIDATION_DIAGNOSTICS !== "true")
        return;
    const items = Array.isArray(req.body?.items) ? req.body.items : [];
    const descriptions = items.map((item, itemIndex) => {
        const description = typeof item === "object" && item !== null && "description" in item
            ? item.description
            : undefined;
        return {
            itemIndex,
            ...
            bytes: typeof description === "string"
                ? Buffer.byteLength(description, "utf8")
                : null,
        };
    });
    const issues = error instanceof zod_1.ZodError
    ...
    console.warn("VALIDATION_DIAGNOSTIC", JSON.stringify({
        method: req.method,
